...
class Login(object):
    ...
    def login_main(self):
        ...
        click_keyi_username = driver.find_element(By.XPATH, "//div[@class='wp-onb-tit']/a[text()='体验用户']")
        self.webdriverwait_click(driver, click_keyi_username)

        login_button = driver.find_element(By.ID, 'code')
        self.webdriverwait_click(driver, login_button)
        time.sleep(1)

        driver.switch_to.frame(driver.find_element(By.ID, 'tcaptcha_iframe'))  # switch 到 滑块frame
        time.sleep(0.5)
        bk_block = driver.find_element(By.XPATH, '//img[@id="slideBg"]')  # 大图
        ...
        slide_block = driver.find_element(By.XPATH, '//img[@id="slideBlock"]')  # 小滑块
        ...
        print("real_position2: ", real_position)
        track_list = self.get_track(real_position + 4)

        ActionChains(driver).click_and_hold(on_element=slid_ing).perform()  # 点击鼠标左键，按住不放
        time.sleep(0.2)

        ActionChains(driver).move_by_offset(xoffset=real_position, yoffset=0).perform()  # 鼠标移动到距离当前位置（x,y）
        # print('第二步,拖动元素')
